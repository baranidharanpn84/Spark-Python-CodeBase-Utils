...
srcfilePath = "s3://" + bucket + "/" + enriched_path + vendor + "/JSON/" + year + "/" + month + "/" + day +"/filesplit_*"
...
dfbaseData = df.select([col for col in df.columns if not col.startswith("xmlns") and not col.startswith("csTradeLine")])
...
dfbaseDataFinal = dfbaseData.select(dfbaseData["*"])

dfbaseDataFinal.write.format("parquet") \
