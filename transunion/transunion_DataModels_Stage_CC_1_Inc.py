...
appNameSuffix = vendor + "_DataModels_Stage_CC1"
...
tgtfilePath = "s3://" + bucket + "/" + enriched_path + vendor + "/Stage/Parquet/creditorContact_1/"
...
dfcreditorContact = reduce(lambda dfcreditorContact, idx: dfcreditorContact.withColumnRenamed(list(dfcreditorContact.schema.names)[idx],
                                                 list(dfcreditorContact.schema.names)[idx].replace("_creditorContact_","")),
            range(len(list(dfcreditorContact.schema.names))),
            dfcreditorContact)

# dfcreditorContact.repartition(sf.col("year"),sf.col("month"),sf.col("day")) \
                   # .write.format("parquet") \
                   # .partitionBy("year","month","day") \
                   # .mode("overwrite") \
                   # .save(tgtfilePath)

dfcreditorContact.write.format("parquet") \
                   .partitionBy("year","month","day") \
                   .mode("overwrite") \
                   .save(tgtfilePath)